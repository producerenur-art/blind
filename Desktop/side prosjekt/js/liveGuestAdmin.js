...
const LiveGuestAdmin = (() => {
  ...
  // Same e-postliste som js/livemix.js sin _isOwner() — bevisst duplisert (ikke
  // delt modul) for å unngå å røre den eksisterende, betalte live-flyten.
  ...
  const GROUPS = [
    { key: 'pending',   title: 'Venter på godkjenning' },
    { key: 'approved',  title: 'Godkjent — kommende' },
    { key: 'live',      title: '🔴 Live nå' },
    { key: 'past',      title: 'Tidligere (avslått / fullført / kansellert)' },
  ];
  ...
  async function render() {
    ...
    if (!_isOwner(cur)) {
      app.innerHTML = `<div style="max-width:600px;margin:3rem auto;text-align:center;color:var(--text2)">Denne siden er kun for eieren.</div>`;
      return;
    }
    app.innerHTML = `
      <div style="max-width:820px;margin:0 auto;padding:1.5rem 1rem 4rem">
        <h1 style="margin:0 0 1.25rem;font-size:1.6rem;font-weight:800">${_I('radio')} Sendeforespørsler</h1>
        <div id="lga-list">Laster…</div>
      </div>`;
    window.scrollTo(0, 0);
    await _load();
  }
  ...
  async function _load() {
    ...
    if (!list) return;
    if (!LiveBroadcastSync._enabled()) { list.innerHTML = `<div style="color:var(--text2);font-size:0.9rem">Krever at skylagring er satt opp (Supabase).</div>`; return; }
    _cache = await LiveBroadcastSync.listPending();
    if (!_cache.length) { list.innerHTML = `<div style="color:var(--text2);font-size:0.9rem">Ingen forespørsler ennå.</div>`; return; }
    ...
    }).join('') || `<div style="color:var(--text2);font-size:0.9rem">Ingen forespørsler ennå.</div>`;
  }
    ...
    return `
      <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:1rem 1.1rem">
        ...
        <div style="display:flex;gap:0.6rem;margin-top:0.6rem">
          <button class="btn btn-primary" onclick="LiveGuestAdmin.approve('${_esc(r.id)}')">${_I('check')} Godkjenn</button>
          <button class="btn btn-ghost" onclick="LiveGuestAdmin.reject('${_esc(r.id)}')">${_I('x')} Avslå</button>
        </div>` : ''}
      </div>`;
  }
  ...
  async function approve(id) {
    ...
    const ok = await LiveBroadcastSync.review(id, 'approved', cur ? cur.username : 'eier');
    if (typeof App !== 'undefined') App.toast(ok ? 'Godkjent!' : 'Kunne ikke godkjenne.', ok ? 'success' : 'error');
    await _load();
  }

  async function reject(id) {
    const reason = (typeof prompt === 'function') ? (prompt('Begrunnelse (valgfritt):') || '') : '';
    const cur = Auth.current();
    const ok = await LiveBroadcastSync.review(id, 'rejected', cur ? cur.username : 'eier', reason);
    if (typeof App !== 'undefined') App.toast(ok ? 'Avslått.' : 'Kunne ikke avslå.', ok ? 'info' : 'error');
    await _load();
  }
